...
def bar1(x, y, w, h):
    """세로 막대 하나 — 한 시각의 값 하나. spark()(곡선 전체)와 짝을 이뤄 공간
    이동(그 슬롯의 값 1개)과 시간 이동(24 h 곡선 전체)의 차이를 글자 없이 보인다.
    2026-09-24(4c 배분, 사용자 메모): "24h랑 Ĉ(t)랑 왜 나눈거지" — '무엇'만 있고
    '왜'가 없던 것을 "막대 대 곡선" 모양 대비로 채운다."""
    return f'<rect x="{x}" y="{y}" width="{w}" height="{h}" fill="{INK}"/>'

def slots(x, y, cell, n, filled):
    """자리 칸 — 채운 칸(검정)과 빈 칸(흰색)을 나란히 그린다. avail_r(=K_r−n_r(t))을
    수식이 아니라 그림으로 보인다. 2026-09-24(4c 배분, 사용자 메모): "avail은 뭐고"
    — 라벨 하나로는 뜻이 안 잡혀 시간 이동 상자 안에 이 칸 그림을 덧붙인다."""
    parts = []
    for i in range(n):
        cx = x + i * (cell + 1.0)
        fill = INK if i < filled else "#fff"
        parts.append(f'<rect x="{cx:.1f}" y="{y}" width="{cell}" height="{cell}" '
...
def fig_architecture():
    ...
    """
    # 2026-09-25(그림 비판 검토): 맨 아래 "동시 실행 수 ≤ availᵣ" 줄을 뺐다(식 (3)과 같은
    # 말 — 그림 안 설명). 그만큼 높이를 줄인다. 선 굵기도 한 단계씩 낮춰(컨테이너
    # 1.7→1.1, 안쪽 상자 1.1→0.8, 화살표 1.3→1.0) 데이터 그림들의 얇은 선과 맞춘다.
    W, H = 215, 218
    L = [head(W, H, "그림 1 CAST 전체 구조")]

    # ── 입력: 탄소 이력(구름)과 작업(문서 묶음) ──
    # 2026-09-25: 기울어진 리본 "N 리전 · 시간별"은 어느 화살표에도 붙지 않고 두 입력
    # 사이에 떠 있었다. 무엇에 대한 말인지(탄소 입력의 단위)를 그 라벨 바로 아래 한 줄로 옮긴다.
    L += [ic_cloud(16, 6, 0.70, "탄소집약도 이력"),
          txt(30.7, 40, "리전별 · 1시간", anchor="middle", size=5.6),
          ic_jobs(148, 8, 0.68, "작업 · 마감")]

    # ── CAST 본체 (둥근 컨테이너) ──
    L += [rbox(10, 54, 195, 116, 10, 1.1),
          txt(18, 67, "CAST", bold=True, size=FB)]

    # 예측 — 신경망 아이콘 + 라벨
    L += [rbox(20, 74, 78, 34, 6, 0.8),
          ic_brain(24, 79, 0.62),
          txt(64, 88, "LSTM 예측", anchor="middle", bold=True, size=FT),
          txt(64, 99, "168 h → 24 h", anchor="middle", size=6.0)]

    # 공간 이동 — 지구 아이콘
    L += [rbox(118, 74, 78, 34, 6, 0.8),
          ic_globe(122, 81, 0.62),
          txt(162, 88, "공간 이동", anchor="middle", bold=True, size=FT),
          txt(162, 99, "ILP · 용량", anchor="middle", size=6.0)]

    # 시간 이동 — 시계 아이콘
    L += [rbox(118, 122, 78, 34, 6, 0.8),
          ic_clock(122, 129, 0.62),
          txt(162, 136, "시간 이동", anchor="middle", bold=True, size=FT),
          txt(162, 147, "마감 · 용량 인지", anchor="middle", size=6.0),
          # 2026-09-24(4c 배분): avail_r(=K_r−n_r(t))을 라벨 하나로만 두지 않고
          # 자리 칸 그림(채운 칸 4·빈 칸 2)을 상자 안에 덧붙인다 — 상자는 안 늘린다.
          slots(152, 150, 2.6, 6, 4)]

    # 입력 → 본체
    # 2026-09-24: 가로 구간이 y=66 이라 "CAST" 글자를 관통했다(fd 가 400dpi 에서 발견).
    # 컨테이너 위쪽(y=50)으로 올려 글자 밖으로 뺀다 — 세로 구간 x=48 은 글자 오른쪽 끝을
    # 4pt 차이로 비껴간다.
    L += [path("M31,44 L31,50 L48,50 L48,74", sw=1.0),
          path("M160,38 L160,50 L186,50 L186,74", sw=1.0)]

    # 값 하나(가는 화살표) vs 곡선 전체(굵은 화살표) — 이 대비가 요지.
    # 2026-09-24(4c 배분): "값 1개" 글자를 빼고 막대 하나(bar1)로 바꾼다 — 아래
    # 시간 이동 쪽 곡선(spark)과 모양으로 대비되어, 같은 예측을 "이 순간 값 하나"
    # (리전을 고르는 문제)로 쓰는지 "곡선 전체"(시각을 고르는 문제)로 쓰는지가
    # 글자를 늘리지 않고도 갈린다.
    # 2026-09-24(4c 재지적, 400% 확인): 막대가 화살표 선에서 떨어져 허공에 떠
    # 보이고, 기준선이 없어 크기가 값으로 안 읽혔다. 아래 "24 h 곡선" 상자와 같은
    # 관례로 작은 네모 상자 안에 넣는다 — 상자 바닥선이 기준선이 되고, 두 상자가
    # [상자: 막대 하나] vs [상자: 곡선+저점 점]으로 나란히 대비된다. LSTM~공간
    # 이동 사이 간격이 18pt뿐이라 곡선 상자(38×16)와 같은 크기는 못 넣었다 —
    # 틀만 같은 관례로 줄여 썼다.
    # 2026-09-24(4c 재재지적, 400% 확인): 막대가 상자 한가운데 떠 있어 바닥선
    # 구실을 못 했다 — 상자 아래 변에 막대 바닥을 붙인다(bar1 y+h = box y+h).
    # 또 상자 위 변(y=91)이 화살촉(marker, 중심 y=88 기준 ±3 정도 퍼짐)과 닿아
    # 있었다 — 상자를 3pt 내려 화살촉과 확실히 띄운다.
    L += [arr(98, 88, 116, 88, 1.0),
          txt(107, 83, "Ĉ(t)", anchor="middle", it=True, size=6.0),
          box(100, 94, 14, 12, sw=0.8), bar1(104, 99, 5, 7)]
    ...
    L += [fat_arrow(59, 108, 59, 132, 5),
          ...
          # 2026-09-24(4c 배분): 곡선의 저점(골짜기)에 점 하나 — 시간 이동이 24 h
          # 곡선 "전체"를 받지만 그 안에서 "한 점을 고른다"는 것을 글자 없이 보인다.
          dot(59, 147, 1.6),
          path("M78,141 L116,141", sw=1.0),
          # 2026-09-25: 글자가 상자 아래 변(y 149)에 걸쳐 있었다 — 변 아래로 내린다.
          txt(59, 157.5, "24 h 곡선", anchor="middle", size=6.0)]

    # 공간 → 시간 (리전 확정)
    # 2026-09-25: 리본이 화살표 위(y 104~116)에 얹혀 공간 이동 상자 아래 변(y 108)을
    # 침범했다. 화살표 왼쪽 빈 자리(두 상자 사이 14pt 틈)로 옮긴다.
    # 틈이 14pt 라 리본(높이 12)을 넣으면 어느 쪽이든 상자 변에 닿는다 — 테두리 없는 글자로.
    L += [arr(157, 108, 157, 120, 1.0),
          txt(153, 117.5, "리전 r(j)", anchor="end", size=6.0)]
    ...
    # 2026-09-24(4c 재지적): availᵣ 라벨(y=119, 상자 밖)과 고리(y=133 시작)가
    # 세로로 14pt 떨어져 있어 라벨이 고리를 가리키는지 한눈에 안 붙었다. 고리를
    # 위로 옮겨 9pt로 좁힌다 — 상자 위 변(y=122)의 둥근 모서리는 x=196 기준
    # y=128까지라 그 아래(곧은 변)에서 시작해야 모서리에 걸리지 않는다. 이보다
    # 더 못 올린다.
    # 2026-09-25: 코드 변수명 availᵣ 대신 본문 식 (12)의 기호 a_r(t)로 맞춘다.
    L += [path("M196,128 L202,128 L202,140 L196,140", sw=0.9, dash="3 1.8"),
          txt(203, 119, "a" + sb("r", "(t)"), anchor="end", it=True, size=6.0)]

    # ── 출력: 리전 랙 ──
    # 2026-09-25: "실행 (리전, 시각)" 리본이 컨테이너 아래 변(y 170)에 걸쳐 있었다.
    # 상자 없이 화살표 오른쪽 글자로 둔다.
    L += [path("M157,156 L157,176", sw=1.0),
          txt(161, 177, "실행 (리전, 시각)", size=6.0)]
    ...
    # 2026-09-24(4c 배분, 사용자 메모): R1 R2 R3 Rn 이 나란히 있어 리전이 넷뿐인
    # 것처럼 보였다. R3와 Rn 사이에 점 셋(생략부호)을 넣어 "그 사이에도 있다"를
    # 그림으로 보인다 — 낱말을 안 늘리려고 문자 대신 dot() 세 번으로 그린다.
    for k, x in enumerate((14, 62, 110)):
        L.append(ic_rack(x, 182, 0.60, "R" + sb(str(k + 1))))
    L += [dot(135, 192, 1.4), dot(141, 192, 1.4), dot(147, 192, 1.4)]
    L.append(ic_rack(166, 182, 0.60, "R" + sb("n")))
    L.append("</svg>")
    return "\n".join(L)
...
def fig_forecast():
    ...
    L += [rbox(6, 8, 203, 50, r=8, sw=1.0),
          ...
          # 2026-09-25: 맑은 고딕(Windows)은 폭이 넓어 왼쪽 테두리를 넘었다 — 크기·위치 조정.
          txt(38, 54, "탄소집약도 과거 168 h", anchor="middle", size=5.3)]
    ...
    # 2026-09-24 4c 배분: "은닉64, 2층이라고 글을 쓸거면 차라리 2층짜리 그림이
    # 낫지 않겠니" 지적과 같은 손질 — 흰10+회색3 타일이 이미 "공통10/기상+3"을,
    # 타일 13장이 이미 "피처13"을 세어 보이므로 그 세 라벨과 리본을 뺀다.
    # 캡션이 이미 "기상 3개 리전만 날씨 3개가 더해지고… 피처 13개"라고 말해
    # 정보 손실이 없다(캡션 수정 불필요).
    # 2026-09-25(그림 비판 검토): 캡션이 짧아지면서 타일이 무엇인지 아무 데도 안 남았다.
    # 두 묶음 아래에 짧은 라벨 한 줄씩만 단다.
    L += [txt(TX + 1.5 * (TW + G) - G / 2, 47, "공통 10", anchor="middle", size=5.6),
          txt(TX + 7 + 4.5 * (TW + G) - G / 2, 47, "기상 +3", anchor="middle", size=5.6)]
    L += [fat_arrow(107, 58, 107, 70, w=6)]

    # ── 본체 ── "은닉 64 · 2층"을 글자가 아니라 셀 두 줄을 쌓아 그림으로
    # 보인다 — 위 타일과 같은 "세어서 읽는" 어법.
    # 2026-09-24 4c 재지적(렌더 확인): 처음 판은 셀을 검게 채워 "바코드 얼룩"으로
    # 보였고(위 타일은 흰 채움+검은 테두리라 같은 그림 안에서 반대 규약이 섞였다),
    # 층 사이 이음선이 그 검은 덩어리에 묻혔고, "64"가 제목 아래 혼자 떠서
    # 무엇을 가리키는지 안 잡혔고, 위(제목·날씨)만 무겁고 가운데가 비었다.
    # 네 가지를 함께 고친다: ① 셀을 타일과 같은 흰 채움+검은 테두리로,
    # ② 20개(10×2)→14개(7×2)로 줄여 덜 빽빽하게, ③ "64"를 셀 줄 바로 왼쪽에
    # 붙여 그 줄을 직접 가리키게, ④ 제목·날씨(글자 구역, 위)와 셀(그림 구역,
    # 아래)을 완전히 분리된 두 층으로 나눠 가운데가 비는 대신 위·아래 두
    # 구역이 균형 있게 상자를 채우게 한다 — 이러면 x축에서 겹칠 일도 없다.
    # 본체를 35→41로 늘리고 그만큼(6pt) 아래 출력 상자를 줄여((5)) 전체
    # 높이(154)는 그대로 유지한다.
    L += [rbox(20, 74, 175, 41, r=10, sw=1.0),   # 2026-09-25: 1.6→1.0 (그림 1과 선 굵기 통일)
          txt(29, 87, "LSTM", size=FB, bold=True),
          txt(189, 87, "+ 미래 24 h 날씨", anchor="end", size=5.8),
          txt(189, 95, "기상 리전만 · 24×3 = 72", anchor="end", size=5.8),
          path("M190,95 L205,95 L205,127 L197,127", sw=0.8, dash="2.5 1.8")]
    # 2026-09-25: "64" 가 혼자 떠 무엇인지 안 읽혔다 — 제목 아래에 "은닉 64 · 2층"으로 쓴다.
    L.append(txt(29, 96, "은닉 64 · 2층", size=5.8))
    CX, CW, CG, CH = 44, 9.0, 1.5, 6.0
    Y1, Y2 = 99, 106.5
    for row_y in (Y1, Y2):
        for k in range(7):
            x = CX + k * (CW + CG)
            L.append(f'<rect x="{x:.1f}" y="{row_y}" width="{CW}" height="{CH}" rx="1.3" '
                      f'fill="#fff" stroke="{INK}" stroke-width="0.8"/>')
    for k in (1, 3, 5):
        x = CX + k * (CW + CG) + CW / 2
        L.append(path(f"M{x:.1f},{Y1+CH:.1f} L{x:.1f},{Y2:.1f}", sw=0.8, head_=False))

    L.append(fat_arrow(107, 115, 107, 124, w=6))

    # ── 출력 ── "향후 24 h/1시간 간격"은 라벨 Ĉᵣ(t…t+24)와 같은 말이라 뺐다.
    # 2026-09-24 4c 재지적: 곡선 진폭을 5로 낮췄더니 거의 직선처럼 보여, "탄소
    # 집약도가 하루 안에서 크게 흔들린다"는 논문의 근거(캘리포니아 최대/최소
    # 3.53배)와 그림이 반대 인상을 줬다. 진폭을 8로 되돌리고, 제목-곡선 여백을
    # 4→3으로 살짝만 줄여 필요한 높이 대부분을 거기서 찾고, 모자란 2pt만
    # 상자·전체 높이를 늘려 채운다(4c 지시: "2~3pt 늘어도 곡선이 골짜기로
    # 보이는 게 더 중요").
    # 2026-09-25: "r" 을 따로 찍은 좌표가 글꼴 폭에 따라 Ĉ 와 겹쳤다 — 한 text 안의
    # tspan 아래첨자(sb 와 같은 방식)로 이어 써 글꼴이 바뀌어도 붙어 다니게 한다.
    L += [rbox(20, 125, 175, 22, r=10, sw=1.0),
          f'<text x="29" y="134" font-size="{FT}"><tspan font-weight="bold">출력  Ĉ</tspan>'
          f'<tspan font-size="5.8" dy="1.5">r</tspan><tspan dy="-1.5"> (t … t+24)</tspan></text>',
          spark(34, 137, 104, 8, n=24)]
