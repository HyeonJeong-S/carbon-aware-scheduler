...
def fig_architecture():
    ...
    # 입력 → 본체
    L += [path("M31,44 L31,66 L48,66 L48,74", sw=1.3),
    ...
    L += [fat_arrow(59, 108, 59, 139, 5),
          path("M62,139 L116,139", sw=1.3),
          f'<rect x="20" y="142" width="52" height="20" fill="#fff"/>',
          box(21, 143, 36, 15, sw=0.8), spark(24, 145, 30, 11),
          txt(59, 155, "24 h 곡선", size=5.4)]
    ...
    # 되먹임 — 잔여 용량 (점선, 바깥으로)
    L += [path("M196,133 L203,133 L203,62 L162,62 L162,72", sw=0.9, dash="3.5 2"),
          f'<rect x="118" y="56" width="80" height="11" fill="#fff"/>',
          txt(158, 65, "availᵣ · 잔여 용량", anchor="middle", it=True, size=5.4)]

    # ── 출력: 리전 랙 ──
    L += [path("M157,156 L157,176", sw=1.4),
          ribbon(108, 170, "실행 (리전, 시각)", 0, 5.2)]
    for k, x in enumerate((14, 62, 110, 158)):
        L.append(ic_rack(x, 182, 0.60, f"R{('₁','₂','₃','\u2099')[k]}"))
...
# ═══════════ 그림 2 · 탄소 예측 (전단) ═══════════
def fig_forecast():
    """그림 2 — LSTM 구조 (2026-09-24 CASPER 작화로 재작성).
    ...
    # ── 본체 ──
    L += [rbox(20, 74, 175, 56, r=10),
          txt(29, 88, "LSTM", size=FB, bold=True),
          ic_brain(30, 94, 1.0)]
    L += [txt(76, 98, "은닉 64 · 2층", size=6.4),
          txt(76, 109, "dropout 0.2", size=6.0),
          txt(76, 122, "마지막 은닉 상태 64", size=6.0)]
    # 기상 리전만 미래 24 h 날씨를 출력 직전에 합류시킨다.
    # 점선은 라벨 오른쪽 바깥(x=205)으로 돌려 글자를 관통하지 않게 한다.
    # 라벨은 상자 안(오른쪽 여백 5pt)에 둔다. 밖으로 빼면 상자 테두리가 글자를 가른다.
    L += [txt(189, 88, "+ 미래 24 h 날씨", anchor="end", size=5.6),
          txt(189, 96, "기상 리전만 · 24×3 = 72", anchor="end", size=5.2),
          path("M188,104 L205,104 L205,150 L197,150", sw=0.8, dash="2.5 1.8")]

    L.append(fat_arrow(107, 130, 107, 140, w=6))

    # ── 출력 ──
    L += [rbox(20, 142, 175, 38, r=10),
          txt(29, 155, "출력  Ĉ", size=FT, bold=True),
          txt(50, 157, "r", size=5.4),
          txt(54, 155, "(t … t+24)", size=FT),
          spark(34, 160, 104, 15, n=24),
          txt(150, 166, "향후 24 h", size=5.8),
          txt(150, 175, "1시간 간격", size=5.4)]

    L.append(txt(6, 193, "리전마다 독립된 모델 8개 · 배치 결정에만 쓰고 배출량 산정은 실측값으로 한다",
                 size=5.6))
    L.append("</svg>")
    return "\n".join(L)
