...
def fig_forecast():
    ...
    # ── 본체 ── dropout·마지막 은닉 상태 두 줄을 뺀 만큼 56→40으로 낮췄다.
    # 아이콘도 0.75로 줄여 "LSTM" 제목 + 아이콘 + 한 줄(은닉 64·2층)만 남는
    # 높이에 맞춘다 — 하이퍼파라미터 세부는 추가자료 A.9 소관.
    L += [rbox(20, 74, 175, 40, r=10),
          txt(29, 86, "LSTM", size=FB, bold=True),
          ic_brain(26, 90, 0.75)]
    L += [txt(78, 100, "은닉 64 · 2층", size=6.4)]
    ...
    L += [txt(189, 86, "+ 미래 24 h 날씨", anchor="end", size=5.6),
          ...
          path("M188,94 L205,94 L205,132 L197,132", sw=0.8, dash="2.5 1.8")]

    L.append(fat_arrow(107, 114, 107, 123, w=6))

    # ── 출력 ── "향후 24 h/1시간 간격"은 라벨 Ĉᵣ(t…t+24)와 같은 말이라 뺐다.
    L += [rbox(20, 124, 175, 30, r=10),
          txt(29, 137, "출력  Ĉ", size=FT, bold=True),
          txt(50, 139, "r", size=5.4),
          txt(54, 137, "(t … t+24)", size=FT),
          spark(34, 142, 104, 10, n=24)]
