...
def fig_forecast():
    ...
    # 보인다 — 위 타일과 같은 "세어서 읽는" 어법. 64개를 다 그릴 순 없어
    # 대표로 10개만 줄마다 그리고, 실제 크기는 "LSTM" 제목 바로 아래 남는
    # 빈 자리에 작은 숫자 하나로만 남긴다(2층은 줄이 둘인 것 자체로 보이므로
    # "2"는 안 적는다). 오른쪽 날씨 라벨(x≥128, y=86·94)과 안 겹치도록 셀
    # 두 줄을 그 아래(y≥97)에, "64"는 그 왼쪽 빈 자리(x=29)에 둔다 — 처음엔
    # "64"를 셀 줄 오른쪽에 리본으로 뒀다가 날씨 라벨과 겹쳐(400% 확인) 옮김.
    L += [rbox(20, 74, 175, 35, r=10),
          txt(29, 86, "LSTM", size=FB, bold=True),
          txt(29, 95, "64", size=6.4)]
    CX, CW, CG, CH = 60, 7.2, 1.2, 4.6
    Y1, Y2 = 97, 103.4
    for row_y in (Y1, Y2):
        for k in range(10):
            x = CX + k * (CW + CG)
            L.append(f'<rect x="{x:.1f}" y="{row_y}" width="{CW}" height="{CH}" '
                      f'rx="1" fill="{INK}"/>')
    for k in (1, 4, 7):
        x = CX + k * (CW + CG) + CW / 2
        L.append(path(f"M{x:.1f},{Y1+CH:.1f} L{x:.1f},{Y2:.1f}", sw=0.7, head_=False))
