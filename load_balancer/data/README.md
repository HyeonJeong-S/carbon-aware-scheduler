...
├── latency_8x8.csv         리전 간 지연 상수 (고정)
├── latency_8x8_normalized.csv    위의 정규화판 (참고용)
...
- `../routed/jobs_routed_*.csv` — 스케줄러 인계용 (jobs.csv + alpha + assigned_region)
- `../framework/results/` — 대시보드 분석용 내부 산출물 (직접 볼 일 없음)
