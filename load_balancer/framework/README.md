...
**1년 실데이터로 동작**: 입력은 전부 `../data/` (jobs.csv + lstm_eval/), 스키마는
그쪽 README 참고.
...
python -m interface.dash_app             # 통합 대시보드 → http://localhost:8050/load-balancer
```

실험을 직접 다시 돌리고 싶을 때 (1년치 전체 ~40분, 대시보드의 '실험 다시 실행' 버튼과 동일):

```bash
python -m load_balancer.framework.run_experiments      # baseline + 고정 α 5개 + α=auto
```

실시간 LSTM 예측을 받아 슬롯 하나를 라우팅하고 JSON으로 내보내기 (서빙 데모, 대시보드 ④탭과 동일):

```bash
python -m load_balancer.framework.realtime_route --t-hour 200                        # jobs.csv 슬롯 재생 → stdout JSON
python -m load_balancer.framework.realtime_route --t-hour 200 --jobs-json my.json --out result.json
```
...
| `run_experiments.py` | baseline + α 스윕 + auto 일괄 실행 → `results/` + `../routed/` |
...
```
data/jobs.csv ─┐
data/lstm_eval/ ┤→ simulator.CarbonSeries ─→ run_experiments ─→ results/ → 통합 대시보드
  (y_true 실측 ·     │   · 탄소 회계 = y_true 적분              └→ routed/ (스케줄러 인계)
