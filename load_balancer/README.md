...
```
load_balancer/
├── docs/          연구 참고자료 (논문 · 논문분석 · 설계 노트) — 파이프라인과 무관
├── data/        ★ 입력: jobs.csv(1년 워크로드) + lstm_eval/(탄소 실측·LSTM 예측)
├── framework/    엔진: 시뮬레이터(simulator.py) · 실험 러너 · 실시간 라우팅
│   └── results/     분석용 산출물 (summary, run별 기록, 시간별 절감량)
└── routed/    ★ 출력: jobs_routed_*.csv — 스케줄러 인계용 납품물
```

각 폴더의 README에 파일별 스키마·규약이 정리되어 있다:
[data](data/README.md) ·
[framework](framework/README.md) ·
[results](framework/results/README.md) ·
...
python -m interface.dash_app                                      # 통합 대시보드 → /load-balancer
python -m load_balancer.framework.run_experiments             # 실험 재실행 (1년치 전체 ~40분)
```
...
5. 결과: `routed/jobs_routed_auto.csv` (+ 고정 α 5개 비교 run)
