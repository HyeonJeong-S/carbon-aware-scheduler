...
def get_forecast_window(carbon_series: dict[str, list[float]], t_now: float,
                        horizon: int = FORECAST_HORIZON) -> dict[str, list[float]]:
...
def mean_carbon(series: list[float], start_hour: float, duration: float, series_len: int) -> float:
    ...
    start_idx = max(0, math.floor(start_hour))
...
def compute_time_shift(job: dict[str, Any], forecast_window: dict[str, list[float]], t_now: float) -> float:
...
def schedule_job(job: dict[str, Any], carbon_series: dict[str, list[float]], mode: str) -> dict[str, Any]:
    ...
    carbon_rate = mean_carbon(actual_series, scheduled_start, duration, len(actual_series))
