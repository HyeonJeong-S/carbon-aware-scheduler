...
class CapacityLedger:
    ...
    def _sweep(self, region):
        ...
        for t, d in ev:
            ...
            spans.append((t, cur))
        over = 0.0
        for (t0, c0), (t1, _) in zip(spans, spans[1:]):
            if c0 > self.cap:
                over += t1 - t0
        return mx, over
...
class _Windows:
    ...
    def actual_mean(self, r, start, dur_h):
        ...
        a = min(max(int(math.floor(start)), 0), n - 1)
        b = min(a + max(1, int(math.ceil(dur_h))), n)
    ...
    def pred_mean(self, r, issue_t, offset, dur_h):
        ...
        i = min(max(int(math.floor(issue_t)), 0), n - 1)
        a = min(max(int(offset), 0), HORIZON - 1)
        b = min(a + max(1, int(math.ceil(dur_h))), HORIZON)
...
def run_rolling(jobs, actual, pred24, capacity, regions,
    ...
    arrivals = {}
    for j in jobs:
        arrivals.setdefault(int(math.floor(j["submit_time"])), []).append(j)
    ...
    for t in range(0, n_hours + 2):
        ...
        for r in regions:
            ...
            picked = set(id(j) for j in S)
            F_ids = set(id(j) for j in F)
