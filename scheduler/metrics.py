...
def compare_modes(results_by_mode: dict[str, list[dict[str, Any]]]) -> dict[str, dict[str, float]]:
    return {mode: aggregate(results) for mode, results in results_by_mode.items()}


def print_comparison(results_by_mode: dict[str, list[dict[str, Any]]]) -> dict[str, dict[str, float]]:
