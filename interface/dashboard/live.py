...
from interface import carbon_history
from interface.dashboard.theme import REGION_COLORS
from interface.regions import LB_TO_REGION, REGION_LABELS, REGIONS, to_iso3
from load_balancer.framework import config as lb_config
from scheduler import data_loader
from scheduler.timeshift import compute_time_shift, mean_carbon
...
# 지도 색 = 24시간 예측 그래프 색 = theme.REGION_COLORS (단일 출처)
MAP_REGION_COLORS = REGION_COLORS
...
MAP_JOBS_CSV = str(lb_config.JOBS_CSV)
MAP_ASSIGN_CSV = str(lb_config.RESULTS_DIR / "assign_alpha_auto.csv")
...
def map_decide(job, actual):
    ...
    dur = job["duration"]
    arr = actual[region]
    imm_c = mean_carbon(arr, job["submit_time"], dur, len(arr)) * dur
    shift_c = mean_carbon(arr, start, dur, len(arr)) * dur
    return {
...
def before_after_totals(running, actual):
    ...
    for d in running:
        ...
        arr = actual[bregion]
        dur = d["duration"]
        before_total += mean_carbon(arr, d["submit_time"], dur, len(arr)) * dur
...
def _hour_bucket_totals(hour):
    ...
    for job in jobs[lo:hi]:
        ...
        arr = actual[bregion]
        dur = d["duration"]
        before += mean_carbon(arr, d["submit_time"], dur, len(arr)) * dur
        after += d["shift_c"]
    return before, after
...
def draw_forecast_chart(t_now):
    ...
    for col in fdf.columns:
        fig.add_trace(go.Scatter(
            ...
            line=dict(width=2, color=REGION_COLORS[col]),
