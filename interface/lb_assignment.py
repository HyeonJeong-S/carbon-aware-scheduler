...
지원하는 CSV 형식 2가지 (컬럼 이름만 다르고 의미는 같음):
  ...
  A) load_balancer/framework/results/assign_*.csv      ← 로드밸런서 공식 산출물
  ...
  C) load_balancer/routed/jobs_routed_*.csv            ← 스케줄러 인계용 납품물
...
def load_assignments(csv_path: str | PathLike) -> dict[str, dict[str, str]]:
...
def attach_to_jobs(jobs: list[dict[str, Any]], assignments: dict[str, dict[str, str]]) -> list[dict[str, Any]]:
